...
class ListTableViewController: UITableViewController {
    ...
    @IBOutlet var listTabbar: UITabBarItem!
...
// Order에 있는 Order을 가지고 와야하는데...
    ...
    override func prepare(for segue: UIStoryboardSegue, sender:Any?) {
        if let destination = segue.destination as? ListTableViewController {
            if let selectedIndex = self.tableView.indexPathForSelectedRows?.first?.row {
                destination.title = Array(order)[selectedIndex]
            }
    }
