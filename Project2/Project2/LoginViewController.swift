...
class LoginViewController: UIViewController, UITextFieldDelegate {
    @IBOutlet var loginTableid: UITextField!
    @IBOutlet var loginPassword: UITextField!
    @IBOutlet var labelStatus: UILabel!

    @IBAction func buttonLogin() {
        if loginTableid.text == "" {
        ...
        let restString: String = "id=" + loginTableid.text! + "&password=" + loginPassword.text!
        ...
        do {
            ...
            if success == "YES" {
                if let name = jsonData["tableid"] as? String {
                    DispatchQueue.main.async {
                        self.labelStatus.text = name + "번 테이블입니다."
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        if textField == self.loginTableid {
